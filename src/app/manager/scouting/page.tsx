...
import { ManagerScoutingHub } from "@/components/manager/scouting/ManagerScoutingHub";

export const dynamic = "force-dynamic";

export default async function ManagerScoutingPage() {
  ...
  const getPosStats = (posArray: typeof players) => {
    if (posArray.length === 0) return { best: null, weakest: null, rating: 0 };
  ...
  if (midPlayers.length > 5) {
    ...
    for (const p of surplusMids) {
      sellOrLoanCandidates.push({
        ...
        reason: `Excess midfield depth (${midPlayers.length} mids). Free up wage budget.`,
      });
    }
  }
  ...
  const immediateStarters = freeAgents.slice(0, 4).map((p) => ({
    id: p.id,
    playerId: p.playerId,
    fullName: p.fullName,
    position: p.position.toUpperCase(),
  ...
  if (upcomingMatch) {
    ...
    if (strengths.length === 0) strengths.push("Well-balanced squad with consistent performance.");
    ...
    nextOpponentReport = {
      ...
      matchPlan: {
        ...
        mainThreat: strengths[0] || "Attacking pace",
        areaToExploit,
        playerToMark,
        ...
        scoutVerdict: isHome
          ? `Playing at HOME gives you advantage. ${vulnerabilities[0] || "Control the pace."}`
          : `AWAY match. Exercise defensive compactness against their ${strengths[0] || "frontline"}.`,
      },
    };
  } else {
    nextOpponentReport = {
